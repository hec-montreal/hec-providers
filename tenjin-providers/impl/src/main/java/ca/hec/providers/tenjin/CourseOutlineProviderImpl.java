...
public class CourseOutlineProviderImpl implements CourseOutlineProvider {
	...
	public Syllabus importSyllabusFromSite(String siteId, String destinationSiteId) throws DeniedAccessException {
		...
		for (COModelInterface e : osylCO.getModeledContent().getChildrens()) {
			...
			if (copyTo != null)
				recursiveCopyToTenjinSyllabus(copyTo, e, lang, templateRules, siteId, destinationSiteId,
						destinationCitationList, copiedResources);
		}
	...
	private void recursiveCopyToTenjinSyllabus(SyllabusCompositeElement elem, COModelInterface comi, String lang,
			HashMap<String, HashMap<String, Object>> templateRules, String originSiteId, String destinationSiteId,
			CitationCollection destinationCitationList, Map<String, String> copiedResources) {
		...
		if (comi instanceof COContentResourceProxy) {
			...
			AbstractSyllabusElement tenjinElement = convertToTenjinElement(cocrp, originSiteId, destinationSiteId,
					copiedResources, lang);
			...
			for (AbstractSyllabusElement r : elem.getElements()) {
				if (r.isComposite() && r.getTitle().equals(rubricTitle)) {
					...
					Long rubricTemplateStructureId = getTemplateStructureIdForElement(parentRubric, tenjinElement,
							templateRules);
					// template structure id could be null if template does not allow that element
					// type
			...
			if (!added) {
				...
				Long newElementTemplateStructureId = getTemplateStructureIdForElement(newRubric, tenjinElement,
						templateRules);
				tenjinElement.setTemplateStructureId(newElementTemplateStructureId);

				if (newRubricTemplateStructureId != null && newElementTemplateStructureId != null) {
			...
			if (compositeElement != null) {
				compositeElement.setTemplateStructureId(
						getTemplateStructureIdForElement(elem, compositeElement, templateRules));
			...
			for (Object child : abstractElement.getChildrens()) {
				...
				if (compositeElement != null) {
					recursiveCopyToTenjinSyllabus(compositeElement, (COModelInterface) child, lang, templateRules,
							originSiteId, destinationSiteId, destinationCitationList, copiedResources);
				} else {
					recursiveCopyToTenjinSyllabus(elem, (COModelInterface) child, lang, templateRules, originSiteId,
							destinationSiteId, destinationCitationList, copiedResources);
				}
			}
		}
	}

	private Long getTemplateStructureIdForElement(SyllabusCompositeElement parentElement,
			AbstractSyllabusElement newElement, HashMap<String, HashMap<String, Object>> templateRules) {
	...
	private void addRubricInCorrectPosition(SyllabusCompositeElement parentElement, SyllabusRubricElement newRubric,
			HashMap<String, HashMap<String, Object>> templateRules) {
		HashMap<String, Object> rulesForParent = null;
		...
		if (element.getType().equals("AssessmentUnit")) {
			...
			if (element.getProperty("assessmentType").equals("intra_exam")
				...
				if (element.getProperties().containsKey("location")) {
					...
					attributes.put("examInClass",
							element.getProperty("location").contains("inClass") ? "true" : "false");
				}
				if (element.getProperties().containsKey("modality")) {
					...
					attributes.put("examWritten",
							element.getProperty("modality").contains("written") ? "true" : "false");
				}
				if (element.getProperties().containsKey("submition_type")) {
					attributes.put("examPaper",
							element.getProperty("submition_type").contains("paper") ? "true" : "false");
					attributes.put("examElectronic",
							element.getProperty("submition_type").contains("elect") ? "true" : "false");
				}
				...
				if (element.getProperties().containsKey("location")) {
					attributes.put("evaluationAtHome",
							element.getProperty("location").contains("home") ? "true" : "false");
					attributes.put("evaluationInClass",
							element.getProperty("location").contains("inClass") ? "true" : "false");
				}
				if (element.getProperties().containsKey("modality")) {
					attributes.put("evaluationOral",
							element.getProperty("modality").contains("oral") ? "true" : "false");
					attributes.put("evaluationWritten",
							element.getProperty("modality").contains("written") ? "true" : "false");
				}
				if (element.getProperties().containsKey("submition_type")) {
					attributes.put("evaluationPaper",
							element.getProperty("submition_type").contains("paper") ? "true" : "false");
					attributes.put("evaluationElectronic",
							element.getProperty("submition_type").contains("elect") ? "true" : "false");
				}
	...
	private AbstractSyllabusElement convertToTenjinElement(COContentResourceProxy element, String originSiteId,
		...
		if (resource.getType().equals("Text")) {
			...
			String uri = resource.getProperty(COPropertiesType.IDENTIFIER, COPropertiesType.IDENTIFIER_TYPE_URI);
			...
			String uri = resource.getProperty(COPropertiesType.IDENTIFIER, COPropertiesType.IDENTIFIER_TYPE_URI);
			String library = resource.getProperty(COPropertiesType.IDENTIFIER,
					COPropertiesType.IDENTIFIER_TYPE_LIBRARY);
			String otherUrl = resource.getProperty(COPropertiesType.IDENTIFIER,
	...
	private String copyResource(String uri, String oldSiteId, String newSiteId)
			throws PermissionException, IdUnusedException, TypeException, InUseException, OverQuotaException,
			IdUsedException, ServerOverloadException, InconsistentException, IdLengthException, IdUniquenessException {
	...
	private String copyCitation(String citationId, String newSiteId, String locale)
		...
		try {
			...
			ContentResourceEdit cre = contentService.addResource(newSiteCollectionId, citationListName, null, 1);
			...
			props.addProperty(ResourceProperties.PROP_DISPLAY_NAME, citationListName);

			contentService.commitResource(cre, NotificationService.NOTI_NONE);
		}

		ContentResource oldListResource = contentService
				.getResource(citationId.substring(0, citationId.lastIndexOf('/')));
